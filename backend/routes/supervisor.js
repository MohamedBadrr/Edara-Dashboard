...
// add new supervisor
router.post('/add-supervisor',admin,
...
body("password").isLength({min : 3 , max : 15}).withMessage("password shold be between 8 - 12 character ") ,
async (req,res) => {
    try {
            // request validation 
...
// update supervisor 
router.put('/update-supervisor/:supervisorId',admin,
body("email").isEmail().withMessage("Please enter the valid Email") ,
...
// delete supervisor
router.delete('/:supervisorId',admin,async (req,res) => {
    try {
        // validate supervisorId
        const query = util.promisify(conn.query).bind(conn); // for multiple query 
        const { supervisorId } = req.params;
